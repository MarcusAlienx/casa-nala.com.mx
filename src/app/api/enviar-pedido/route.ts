...
export async function POST(request: Request) {
  try {
    ...
    const { carrito, datosCliente, opcionEntrega, direccionEntrega, totalPedido } = payload;
    ...
    const transporter = nodemailer.createTransport({
      ...
      port: parseInt(process.env.EMAIL_SMTP_PORT || "465", 10), // Puerto SMTP
    ...
    return NextResponse.json({ message: 'Pedido enviado por correo exitosamente.' }, { status: 200 });
  } catch (error) {
    ...
    return NextResponse.json({ message: 'Error al procesar el pedido.', error: errorMessage }, { status: 500 });
  }
}
