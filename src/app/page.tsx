...
const opcionesEntrega = [
  ...
  { id: 'ubereats', nombre: 'Uber Eats', costo: 0, logo: '/logos/uber_eats.avif' },
...
const IconMapPin = ({ className = "w-4 h-4" }: { className?: string }) => (
  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className={className}>
    <path fillRule="evenodd" d="M9.69 18.933l.003.001C9.89 19.02 10 19 10 19s.11.02.308-.066l.002-.001zm.612-1.426a5.02 5.02 0 00-7.002 0C3.503 17.968 3 18.495 3 19s.503 1.032.902.574a4.999 4.999 0 007.196 0c.399.458.902.074.902-.574s-.503-1.032-.902-.574zM10 2a5 5 0 00-5 5c0 3.564 4.204 8.09 5 9.434 1.058-1.668 5-6.168 5-9.434a5 5 0 00-5-5zM10 9a2 2 0 100-4 2 2 0 000 4z" clipRule="evenodd" />
  </svg>
);
...
export default function Home() {
  ...
  const [datosCliente, setDatosCliente] = useState({
    nombre: '', telefono: '', horario: '', comentarios: ''
  });
  ...
  const [direccionEntrega, setDireccionEntrega] = useState({
    calle: '', numero: '', colonia: '', codigoPostal: '', referencias: ''
  });

  // --- useEffect para cargar el menú desde la API ---
  useEffect(() => {
    if (isClient) { // Solo cargar en el cliente
      const fetchMenu = async () => {
        ...
        try {
          ...
          setMenuError(error instanceof Error ? error.message : 'Error desconocido al cargar el menú.');
        } finally {
  ...
  const agregar = (platillo: ProcessedMenuItem) => { // Tipo de platillo actualizado
    setCarrito((prev) => {
  ...
  const quitar = (platillo: ProcessedMenuItem) => { // Tipo de platillo actualizado
    setCarrito((prev) => {
      const existe = prev.find((item) => item.id === platillo.id);
      if (!existe) return prev;
      if (existe.cantidad === 1) return prev.filter((item) => item.id !== platillo.id);
      return prev.map((item) =>
        item.id === platillo.id ? { ...item, cantidad: item.cantidad - 1 } : item
      );
    });
  ...
  const irAlInicio = () => { setPaso('menu'); window.scrollTo({ top: 0, behavior: 'smooth' }); };
  const continuarAPaso2 = () => { if (carrito.length > 0) { setPaso('datos'); window.scrollTo({ top: 0, behavior: 'smooth' }); } };
  const confirmarPedido = async () => { // <--- Añadido async aquí
    if (!datosCliente.nombre || !datosCliente.telefono || !datosCliente.horario) return;
    if (opcionEntrega === 'domicilio' && (!direccionEntrega.calle || !direccionEntrega.numero || !direccionEntrega.colonia || !direccionEntrega.codigoPostal)) {
    ...
    const orderPayload = {
      ...
      datosCliente,
      opcionEntrega,
      direccionEntrega: opcionEntrega === 'domicilio' ? direccionEntrega : undefined,
    ...
    try {
      ...
      if (response.ok) {
        ...
        alert('Hubo un problema al enviar tu pedido por correo. Por favor, intenta notificarlo por WhatsApp o contacta al restaurante.');
      ...
      alert('Error de conexión al intentar enviar tu pedido. Por favor, revisa tu conexión e inténtalo de nuevo.');
    }
  };
  const volverAMenu = () => { setPaso('menu'); window.scrollTo({ top: 0, behavior: 'smooth' }); };
  const vaciarCarrito = () => {
    setCarrito([]); setPaso('menu');
    setDatosCliente({ nombre: '', telefono: '', horario: '', comentarios: '' });
    setOpcionEntrega('recoger');
    setDireccionEntrega({ calle: '', numero: '', colonia: '', codigoPostal: '', referencias: '' });
    window.scrollTo({ top: 0, behavior: 'smooth' });
  };
  ...
  return (
    <main className="min-h-screen bg-[#F5E8D2] pb-24">
      {/* Header Modificado para Responsividad */}
      <header className="sticky top-0 z-40 w-full bg-[#6C3A3A] text-[#F5E8D2] shadow-md">
        <div className="p-4 flex justify-between items-center"> {/* Este div define la altura del header por su padding y contenido */}
          {/* Logo y Nombre */}
          {/* flex-grow en móvil para permitir que el texto se centre. No self-stretch ni h-full en este div. */}
          <div className="flex flex-grow items-center gap-2 md:flex-grow-0 md:gap-4">
            <Image
              ...
              height={40} 
              alt="Logo Casa Nala"
              className="bg-[#F5E8D2] p-0.5 h-12 w-auto object-contain flex-shrink-0" // Altura ajustada a h-12 (48px)
            />
            {/* Contenedor del nombre para centrar en móvil */}
            <div className="flex-grow text-center md:text-left">
              <div className={`text-xl md:text-2xl tracking-wider ${poiretOne.className}`}>CASA NALA</div>
              <div className={`text-xs md:text-sm italic text-[#F5E8D2] font-light leading-none mt-0.5 ${amaticSC.className}`}>
                Comida de México
              </div>
            </div>
          </div>
          ...
          {/* flex-shrink-0 para evitar que se encoja. items-center en el padre se encarga de la alineación vertical. */}
          <nav className="hidden md:flex items-center gap-4 lg:gap-6 text-sm lg:text-lg flex-shrink-0">
            <button onClick={() => { irAlInicio(); setIsMobileMenuOpen(false); }} className="hover:underline font-medium" type="button">Menú</button>
            <a href="#pedidos" onClick={() => setIsMobileMenuOpen(false)} className="hover:underline">Pedidos</a>
            <a href="/reservaciones" onClick={() => setIsMobileMenuOpen(false)} className="hover:underline">Reservaciones</a>
            <a href="/ubicacion" onClick={() => setIsMobileMenuOpen(false)} className="hover:underline">Ubicación</a>
          </nav>
        ...
        {isMobileMenuOpen && (
          <nav className="md:hidden bg-[#6C3A3A] text-[#F5E8D2] py-2 shadow-lg">
            <button onClick={() => { irAlInicio(); setIsMobileMenuOpen(false); }} className="block w-full text-left px-4 py-2 hover:bg-[#9DA17B]" type="button">Menú</button>
            <a href="#pedidos" onClick={() => setIsMobileMenuOpen(false)} className="block px-4 py-2 hover:bg-[#9DA17B]">Pedidos</a>
            <a href="/reservaciones" onClick={() => setIsMobileMenuOpen(false)} className="block px-4 py-2 hover:bg-[#9DA17B]">Reservaciones</a>
            <a href="/ubicacion" onClick={() => setIsMobileMenuOpen(false)} className="block px-4 py-2 hover:bg-[#9DA17B]">Ubicación</a>
          </nav>
        )}
      </header>
      ...
      <section className="bg-[#F5E8D2] flex flex-col items-center text-[#6C3A3A] py-6">
        <div className={`mt-2 text-4xl font-light tracking-[0.15em] ${poiretOne.className}`}>CASA NALA</div>
        <div className={`text-xl italic mt-1 text-[#9DA17B] ${amaticSC.className}`}>Comida de México</div>
      </section>
      ...
      {paso === 'menu' && (
        <section id="menu-completo" className="max-w-6xl mx-auto px-4">
          ...
          {isLoadingMenu && (
            <div className="text-center py-10 text-[#6C3A3A]">Cargando menú...</div>
          )}
          {menuError && (
            <div className="text-center py-10 text-red-600">Error al cargar el menú: {menuError}</div>
          )}
          {/* --- Fin Indicador --- */}
            ...
            <>
              ...
              <div className="sticky top-[84px] z-30 bg-[#F5E8D2] py-3 mb-2 shadow-sm">
                ...
                {activeCategory && menuData[activeCategory] && (
                  <div className="max-w-3xl mx-auto flex justify-start space-x-3 px-4 overflow-x-auto whitespace-nowrap mt-1 pt-1 border-t border-[#D9D4CE]/50"> {/* Cambiado justify-center a justify-start */}
                    {Object.keys(menuData[activeCategory]).map((subCategoriaKey) => (
                      <button
                        key={subCategoriaKey}
                        onClick={() => {
                          if (menuData && activeCategory && menuData[activeCategory] && menuData[activeCategory][subCategoriaKey]) {
                            const itemsInSubCategory = menuData[activeCategory][subCategoriaKey];
                            if (itemsInSubCategory.length > 0) {
                              const firstItemId = itemsInSubCategory[0].id;
                              const elementToScrollTo = document.getElementById(`platillo-${firstItemId}`);
                              if (elementToScrollTo) {
                                ...
                                console.log(`Elemento platillo-${firstItemId} no encontrado para scroll.`);
                              }
                            }
                          }
                        }}
              ...
              <div ref={menuContainerRef}>
                {Object.entries(menuData).map(([categoriaPrincipal, subMenuDataObject]) => {
                  // Aplanar todos los platillos de esta categoría principal
                  const todosLosPlatillosDeCategoria = Object.values(subMenuDataObject).flat();
                  
                  return (
                    <section key={categoriaPrincipal} id={categoriaPrincipal} className="mb-12 pt-4">
                      <h2 className={`text-3xl font-bold text-[#6C3A3A] tracking-wider mb-6 border-b-2 border-[#D7A556] pb-2 ${poiretOne.className}`}>
                        {categoriaPrincipal.replace(/_/g, ' ')}
                      </h2>
                      {/* Anclas invisibles para subcategorías si se quieren usar para scroll */}
                      {Object.keys(subMenuDataObject).map(subCategoriaKey => (
                        <div key={`${categoriaPrincipal}-${subCategoriaKey}-anchor`} id={`${categoriaPrincipal}-${subCategoriaKey.replace(/\s+/g, '_')}-anchor`} />
                      ))}
                      <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                        {todosLosPlatillosDeCategoria.map((platillo) => (
                          ...
                          >
                            <Image
                              src={platillo.image || '/images/menu/logonala-menu.avif'} // Fallback image
                              width={150}
                              height={150}
                              alt={platillo.nombre}
                            ...
                            <div className="flex-grow flex flex-col">
                              <h3 className="font-semibold text-lg text-[#6C3A3A] mb-1">{platillo.nombre}</h3>
                              {platillo.detalles && (
                                <p className="text-xs text-gray-600 mb-2 flex-grow">{platillo.detalles}</p>
                              )}
                              <div className="mt-auto">
                                <div className="font-bold text-[#D7A556] text-xl mb-3">
                                  {typeof platillo.precio === 'number' && !isNaN(platillo.precio)
              ...
              <select
                ...
                onChange={e => {
                  ...
                  if (newOpcionEntrega === 'domicilio') {
                    ...
                    setDatosCliente(prev => ({ ...prev, horario: 'DOMICILIO_ASAP' })); 
                  } else {
              ...
              >
                {opcionesEntrega.map(opcion => (
                  <option key={opcion.id} value={opcion.id}>
                    ...
                    {opcion.id === 'domicilio' ? "Servicio a Domicilio +$40" : opcion.nombre}
                  </option>
                ))}
              </select>
            </div>
      ...
      {paso === 'confirmacion' && (
        <section className="max-w-3xl mx-auto px-6 py-8 bg-white rounded-xl shadow-md mt-4 text-center">
          ...
          <p className="text-[#9DA17B] mb-2">Gracias {datosCliente.nombre}, tu pedido ha sido procesado.</p>
          <p className="text-sm text-gray-600 mb-6">Recibirás (o hemos intentado enviar) una copia por correo electrónico. También puedes notificarlo por WhatsApp.</p>
          ...
          <a
            href={`https://wa.me/${process.env.NEXT_PUBLIC_WHATSAPP_NUMBER || '5213332531118'}?text=${encodeURIComponent(
              `¡Hola Casa NALA! Quiero confirmar mi pedido:\n\nCliente: ${datosCliente.nombre}\nTeléfono: ${datosCliente.telefono}\nHorario: ${datosCliente.horario}\nOpción Entrega: ${opcionEntrega}\n${
                ...
                .join('\n')}\n${
                datosCliente.comentarios ? `Comentarios: ${datosCliente.comentarios}\n` : ''
            ...
            <ul className="space-y-2 text-left">
              {carrito.map(item => (
                <li key={item.id} className="flex justify-between">
                  <span>{item.nombre} {item.detalles ? `(${item.detalles.substring(0, 20)}${item.detalles.length > 20 ? '...' : ''})` : ''} x {item.cantidad}</span>
          ...
          <div className="bg-[#F5E8D2]/50 p-4 rounded-lg mb-6 text-left">
            <h3 className="font-bold text-[#6C3A3A] mb-2">Detalles del Pedido:</h3> {/* Texto cambiado aquí */}
          ...
          <div className="mt-8 pt-6 border-t border-gray-300">
            <h3 className={`text-xl font-semibold text-center text-[#6C3A3A] mb-4 ${poiretOne.className}`}>¿Tienes dudas o un pedido especial?</h3>
            <p className="mb-4 text-center text-gray-700 text-sm">
              En Casa Nala estamos encantados de escucharte. Nuestro compromiso es ofrecerte el auténtico sabor de México con atención cercana y de calidad.
            </p>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 text-center text-xs mb-6">
              <div>
                ...
                <p className="font-semibold text-sm text-[#6C3A3A]">Visítanos</p>
                <p className="text-gray-600">Av. Miguel López de Legaspi & Vasco de Gama,<br/>Col. 18 de Marzo, 44960 Guadalajara, Jal.</p>
                <a 
              ...
              <div>
                ...
                <a href="https://wa.me/523332531118" target="_blank" rel="noopener noreferrer" className="text-[#D7A556] hover:underline">
                  +52 33 3253 1118
                </a>
              </div>
              ...
              <div>
                ...
                <a href="mailto:ordenes@casanala.com.mx" className="text-[#D7A556] hover:underline">
            ...
            <div className="text-center text-xs text-gray-600 mb-4">
              <p className="flex items-center justify-center">
                ...
                Sitio Web: 
                <a href="https://casanala.com.mx" target="_blank" rel="noopener noreferrer" className="ml-1 text-[#D7A556] hover:underline">
            ...
            <p className="text-center text-gray-700 text-sm">
              Ya sea para eventos, pedidos para llevar o simplemente por antojo, en Casa Nala estamos para servirte. ¡Gracias por elegirnos!
            </p>
          </div>
        </section>
      )}
      ...
      {opcionEntrega === 'domicilio' && paso === 'datos' && showDeliveryInfoModal && (
            ...
            <div className="aspect-video mb-4 rounded-lg overflow-hidden relative">
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-between p-4 bg-black bg-opacity-30 rounded-lg"> 
                <p className="text-white text-lg md:text-xl text-center font-semibold">
                  Entregamos en un radio de 5km alrededor de nuestro restaurante.
                </p>
                <p className="text-gray-100 text-xs md:text-sm text-center mt-2">
                  Por ahora, nuestro servicio a domicilio está limitado a las siguientes colonias: Colón Industrial, Residencial Victoria, Jardines de la Cruz, Jardines de San Jose, Lomas de Polanco, Residencial Victoria, Loma Bonita.
                </p>
              </div>
            </div>
      ...
      <section className="max-w-3xl mx-auto my-8 p-6 bg-white rounded-xl shadow-lg">
        ...
        <div className="flex flex-wrap justify-center gap-8 items-center"> {/* Added items-center */}
          <a href="https://www.ubereats.com/mx" target="_blank" rel="noopener noreferrer" className="transition hover:scale-105">
            <Image
          ...
          <a href="https://www.rappi.com.mx/" target="_blank" rel="noopener noreferrer" className="transition hover:scale-105">
            <Image
          ...
          <a href="https://mexico.didiglobal.com/food/" target="_blank" rel="noopener noreferrer" className="transition hover:scale-105">
            <Image
